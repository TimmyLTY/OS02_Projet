...
void advance_time_omp( const fractal_land& land, pheronome& phen,
                       ...
                       double& time_ants_ms, double& time_evap_ms, double& time_update_ms )
{
    ...
    advance_all_ants_omp(ants, phen, land, pos_food, pos_nest, cpteur);
    auto t1 = clock_t_hr::now();
    phen.do_evaporation_omp();  // Version OpenMP parallélisée
...
int main(int nargs, char* argv[])
{
    ...
    bool headless = false;
    int num_threads = 0; // 0 = défaut OMP
    for (int a = 1; a < nargs; ++a) {
        ...
        if (arg == "--threads" || arg == "-t") {
            if (a + 1 < nargs) num_threads = std::stoi(argv[++a]);
        }
    }
    ...
    if (!headless) {
        win_ptr = new Window("Ant Simulation (OpenMP)", 2*land.dimensions()+10, land.dimensions()+266);
    ...
    // Déterminer le nombre réel de threads
    ...
    std::cout << "=== Mesure des performances (OpenMP) ===" << std::endl;
    ...
    std::cout << "Threads OpenMP : " << actual_threads << std::endl;
    if (max_iterations > 0)
    ...
    while (cont_loop) {
        ...
        advance_time_omp( land, phen, pos_nest, pos_food, ants, food_quantity,
    ...
    std::cout << "\n=============================================" << std::endl;
    std::cout << "  RAPPORT DE PERFORMANCE (OpenMP, " << actual_threads << " threads)" << std::endl;
    std::cout << "=============================================" << std::endl;
