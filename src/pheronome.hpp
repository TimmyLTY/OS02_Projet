...
public:
    ...
    /**
     * @brief Évaporation parallélisée avec OpenMP
     */
    void do_evaporation_omp( ) {
    ...
    // --- Accesseurs pour MPI (Phase 4A) ---
    // Expose raw double pointers for MPI_Allreduce / MPI_Allgatherv
    double* get_buffer_data() {
        return reinterpret_cast<double*>(m_buffer_pheronome.data());
    }
    double* get_map_data() {
        return reinterpret_cast<double*>(m_map_of_pheronome.data());
    }
    const double* get_buffer_data() const {
        return reinterpret_cast<const double*>(m_buffer_pheronome.data());
    }
    const double* get_map_data() const {
        return reinterpret_cast<const double*>(m_map_of_pheronome.data());
    }
    std::size_t buffer_size_doubles() const {
        return m_stride * m_stride * 2;
    }
    std::size_t get_stride() const { return m_stride; }
    std::size_t get_dim() const { return m_dim; }
